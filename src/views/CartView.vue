<template>
   <div class="pt-3 px-4 pb-72">
      ...
      <div class="grid gap-4 mt-6" >
         <div
            class="grid grid-cols-9 bg-white py-3 px-4 rounded-lg gap-4 items-center"
            v-for="cart in carts"
            :key="cart.id"
         >
            <div
               class="col-span-3 w-[94px] h-[94px] flex justify-center items-center bg-[#F0F0F2] rounded-md"
            >
               <img
                  class="h-16"
                  :src="'/assets/images/' + cart.products.img"
                  alt=""
               />
            </div>
            <div class="col-span-4 flex flex-col">
               ...
               <h6 class="product-price">
                  Rp. {{ cart.products.price * cart.order_quantity }}
               </h6>
            </div>
            <h6
               @click="deleteCart(cart.id)"
               class="col-span-2 ml-auto text-red-400 cursor-pointer"
            >
               <i
                  class="bx bx-trash text-2xl bg-[#f9f9f9] p-2 rounded-full"
               ></i>
            </h6>
         </div>
      </div>
      ...
      >
         ...
         <table class="mb-2">
            ...
            <tr class="border-t-2">
               ...
               <td class="text-right product-price pt-4">
                  Rp. {{ subTotal + 10000 }}
               </td>
            </tr>
         </table>

         <button class="bg-primary py-3 px-4 text-white rounded-lg mt-4">
            Check Out
         </button>
      </div>
